...
* xx should this be a sub-ado?
*-------------------------------------------------------------------------------

* First: Check to see if we have Python, and it is version 3.5 or arbove
qui python query
...
* XX check versioning here

* Check to see if we have NumPy
cap python which numpy
if _rc!=0 {
	di as error "Error: Could not import the Python module numpy. "
	di as error: "Run the command {pyforest_setup:pyforest_setup} to try installing this from Stata, and then restart Stata."
	exit 1
}
